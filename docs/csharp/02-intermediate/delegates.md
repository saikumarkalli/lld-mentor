...
- [Events](./events.md) — Events are delegates with `add`/`remove` access modifiers and publisher-only invocation
- [LINQ](./linq.md) — Entire LINQ is built on `Func<T, bool>`, `Func<T, TResult>` delegates
- [Async/Await](../03-advanced/async-await.md) — `Task.ContinueWith` uses `Func<Task, T>` continuation delegates
- **Strategy Pattern** in → [`LLDMaster.Patterns/03_Behavioral/12_Strategy/`](../../LLDMaster.Patterns/03_Behavioral/12_Strategy/)
- [Observer Pattern](../../LLDMaster.Patterns/03_Behavioral/11_Observer/) — Events implement the Observer pattern
