...
## 🔗 Connected Topics
- [Value Types vs Reference Types](./value-vs-reference-types.md) — The "what" to this topic's "where"
- [Nullable Types](./nullable-types.md) — `Nullable<T>` wraps value types with heap semantics
- [Garbage Collection](../04-expert/garbage-collection.md) — The GC that manages the heap
- [Span\<T\> & Memory\<T\>](../03-advanced/span-memory.md) — Zero-allocation stack-based processing
